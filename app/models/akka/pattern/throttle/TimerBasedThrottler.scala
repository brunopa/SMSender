package models.akka.pattern.throttle
...
class TimerBasedThrottler(var rate: Rate) extends Actor with Throttler with LoggingFSM[State, Data] {
  ...
  initialize
  private def startTimer(rate: Rate) = setTimer("morePermits", Tick, FiniteDuration(rate.durationInMillis(), TimeUnit.MILLISECONDS), true)
